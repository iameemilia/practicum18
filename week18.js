...
function makeOne() {
	...
	console.log('Ключ "games" со значением 11 был записан в Local Storage.');
}
...
function makeThree() {
	localStorage.setItem('personData', 'name=John, age=25');
...
const showEight = () => {
	...
	if (objectEightString) {
		...
		resultElement.textContent = `${objectEight.value};`;
	} else {
...
const removeFromCart = () => {
	...
	if (cartItems.length > 0) {
		...
		console.log('Массив корзины пуст.');
	}
};
...
const showCart = () => {
	...
	practicum23.textContent = cartItems.join(', ');
};
...
const updateCartCount = () => {
	...
	practicum24.textContent = `Количество товаров в корзине ${cartItems.length}`;
};
